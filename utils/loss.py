...
# from Density_aware_Chamfer_Distance.utils_v2.model_utils import calc_dcd
...
def CVAE_loss(recon_x, x, mean, log_var, loss_type, l2_weight=1):
    ...
    if loss_type == 'CD':
        aux_loss, _ = chamfer_distance(recon_x.permute(
            0, 2, 1), x.permute(0, 2, 1), point_reduction='sum')
    elif loss_type == 'EMD':
        aux_loss = earth_mover_distance(
            recon_x, x, transpose=True).sum() / x.size(0)
...
def CVAE_loss_mano(recon_x, x, mean, log_var, loss_tpye, mode='train'):
    ...
    if loss_tpye == 'L2':
        ...
        recon_loss = earth_mover_distance(
            recon_x, x, transpose=False).sum() / x.size(0)
    elif loss_tpye == 'DCD':
        recon_loss, _, _ = calc_dcd(recon_x, x, alpha=200, n_lambda=0.5)
...
def CMap_loss(obj_xyz, hand_xyz, cmap):
    ...
    hand_xyz_prior = hand_xyz[:, prior_idx, :]

    B = obj_xyz.size(0)
    T = cmap.size(2)

    # [B, N1] NN distance from obj pc to hand pc
    obj_CD, _ = utils_loss.get_NN(obj_xyz, hand_xyz_prior)
    ...
    for i in range(B):
        ...
        for j in range(T):
            ...
            if n_points == 0:  # skip nan
                continue
            tmp_list.append(obj_CD[i][mask].sum() /
                            n_points)  # point reduction
...
def CMap_loss2(obj_xyz, hand_xyz):
    ...
    # point and batch reduction
    return 20.0 * obj_CD[obj_cmap].sum() / cpoint_num
...
def O2O_CMap_loss(obj_xyz, hand_xyz, cmap):
    '''
    # prior cmap loss on gt cmap
    :param obj_xyz: [B, N1, 3]
    :param hand_xyz: [B, N2, 3]
    ...
    # compute contact map loss
    n_points = torch.sum(cmap)
    # o2o_loss = o2o_CD[cmap].sum() / (B * n_points)
    o2o_loss = obj_xyz[:, :, 4][cmap]
    return -3000.0 * o2o_loss.sum() / (B * n_points) / 100
...
def CMap_loss4(obj_xyz, hand_xyz, cmap):
    ...
    # [B, N1] NN distance from obj to hand
    obj_CD, _ = utils_loss.get_NN(obj_xyz, hand_xyz_prior)
    # [B, N2] NN distance from hand to obj
    hand_CD, _ = utils_loss.get_NN(hand_xyz_prior, obj_xyz)
    # compute contact map loss
    n_points = torch.sum(cmap)
...
def H2O_consistency_loss(recon_dists, gt_dists):
    '''
    :param recon_hand_xyz: [B, N2, 3]
    :param gt_hand_xyz: [B, N2, 3]
    :param obj_xyz: [B, N1, 3]
    ...
    # hard cmap
    recon_cmap = recon_dists > 0.9
    gt_cmap = gt_dists > 0.9
    gt_cpoint_num = gt_cmap.sum() + 0.0001
    consistency = (recon_cmap * gt_cmap).sum() / gt_cpoint_num
    # soft cmap
    #consistency2 = torch.nn.functional.mse_loss(recon_dists, gt_dists, reduction='none').sum() / recon_dists.size(0)
...
def CMap_consistency_loss_soft(recon_hand_xyz, gt_hand_xyz, obj_xyz):
    ...
    consistency = torch.nn.functional.mse_loss(
        recon_dists, gt_dists, reduction='sum') / recon_hand_xyz.size(0)
    return consistency


def contact_map_consistency(h2o_recon, recon_hand_xyz, obj_xyz):
    recon_dists, _ = utils_loss.get_NN(obj_xyz, recon_hand_xyz)  # [B, N1]
    normalize_recon_dists = 1 - 2 * (torch.sigmoid(2 * 100 * recon_dists) - 0.5)
    consistency = torch.nn.functional.mse_loss(
        normalize_recon_dists, h2o_recon, reduction='mean')
    return consistency
...
def inter_penetr_loss(hand_xyz, hand_face, obj_xyz, nn_dist, nn_idx):
    ...
    interior = utils_loss.get_interior(
        hand_normal, hand_xyz, obj_xyz, nn_idx).type(torch.bool)  # True for interior
    penetr_dist = nn_dist[interior].sum() / B  # batch reduction
    return 100.0 * penetr_dist

def contact_inter_penetr_loss(hand_xyz, hand_face, obj_xyz, nn_dist, nn_idx, cmap):
    '''
    get penetrate object xyz and the distance to its NN
    :param hand_xyz: [B, 778, 3]
    :param hand_face: [B, 1538, 3], hand faces vertex index in [0:778]
    ...
    penetr_dist = (nn_dist[interior] * (1 + 3 * reverse_cmap[interior])).sum() / B
    return 100.0 * penetr_dist

def penetration_loss(hand_xyz, hand_face, obj_xyz, nn_dist, nn_idx):
    '''
    get penetrate object xyz and the distance to its NN
    :param hand_xyz: [B, 778, 3]
    :param hand_face: [B, 1538, 3], hand faces vertex index in [0:778]
    ...
    interior = utils_loss.get_interior(
        hand_normal, hand_xyz, obj_xyz, nn_idx).type(torch.bool)  # True for interior
    penetr_dist = nn_dist[interior].sum() / (interior.sum()+ 0.0001)
    return 100 * penetr_dist
...
def TTT_loss(hand_xyz, hand_face, obj_xyz, cmap_affordance, cmap_pointnet, threshold = 0.05):
    '''
    :param hand_xyz:
    :param hand_face:
    :param obj_xyz:
    ...
    # hand-centric loss
    contact_loss = 2.5 * Contact_loss(obj_xyz, hand_xyz, cmap=nn_dist < threshold**2)
