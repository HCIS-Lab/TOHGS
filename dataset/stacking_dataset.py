...
class grasping_pose(Dataset):
    def __init__(self, mode = "train", batch_size=160, root = 'task_oriented_grasps_dataset/stacking', sample_points= 2048):
        self.root = root
        self.batch_size = batch_size
        self.mode = mode
        self.sample_points = sample_points
        if self.mode == 'train':
            self.obj_list = ['A', 'C', 'D', 'E', 'G', 'H', 'J', 'L', 'M', 'O', 'P', 'Q', 'S', 'T', 'U', 'W', 'X']
        elif self.mode == 'val':
            self.obj_list = ['F', 'I', 'K', 'N', 'R', 'V']
        # self.obj_list = os.listdir(self.root)
        self.init = []
        self.goal = []
        self.recon_param = []
        for obj in self.obj_list:
            path = os.path.join(self.root, obj + '.npy')
            config = np.load(path, allow_pickle = True).item()
            for i in range(len(config['init'])):
                self.init.append(config['init'][i])
                self.goal.append(config['goal'][i])
                self.recon_param.append(config['recon_parms'][i])
        self.dataset_size = len(self.init)
    ...
    def __getitem__(self, idx):
        init_config = self.init[idx]
        goal_config = self.goal[idx]
...
class scene(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        origin_grasped_obj_pcd = (init_grasped_obj_pcd.T - init_trans).T
        ...
        hand_vertex = hand_vertex[0].T
        recon_param_original = recon_param.clone()
        recon_param_original[:, :3] -= torch.FloatTensor(init_trans).reshape(-1)
        obstacles_pcd_list = [torch.FloatTensor(table_pcd)]
        init_based_obj_pcd = trimesh.sample.sample_surface(based_obj_mesh, self.sample_points)[0]
        init_based_obj_pcd = np.matmul(init_based_pose[:3, :3], init_based_obj_pcd.T) + init_based_pose[:3, 3].reshape(-1, 1)
        obstacles_pcd_list.append(torch.FloatTensor(init_based_obj_pcd.T))        
        ...
        origin_hand_vertex, _ = self.manolayer.forward(
                th_trans=recon_param_original[:, :3],
                th_pose_coeffs=recon_param_original[:, 3:],
            )
        origin_hand_vertex = origin_hand_vertex[0].T
        if self.mode == 'train':
            orient = torch.FloatTensor(1, 3).uniform_(-np.pi, np.pi)
            hand_mat = np.eye(4)
            aug_rot_mats = batch_rodrigues(orient.view(-1, 3)).view([1, 3, 3])
            ...
            hand_vertex = aug_trans @ hand_vertex
            init_grasped_obj_pcd = aug_trans.numpy() @ init_grasped_obj_pcd
            origin_grasped_obj_pcd = aug_trans.numpy() @ origin_grasped_obj_pcd
            ...
            origin_hand_vertex = origin_hand_vertex[0].T
        return {
                "init_hand_verts": hand_vertex.T / 1000,
