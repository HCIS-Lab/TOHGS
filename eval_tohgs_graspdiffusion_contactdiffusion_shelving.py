...
def get_json(filename):
    ...
    data = json.load(f)
    goal = []
    for i in data['goal_pose']:
        goal.append(i)
    init = []
    for i in data['init_pose']:
        init.append(i)
    obstacle_name = []
    for i in data['obstacle_name']:
        obstacle_name.append(i)
    obstacle_pose = []
    for i in data['obstacle_pose']:
        obstacle_pose.append(i)
    shelf_name = []
    for i in data['shelf_name']:
        shelf_name.append(i)
    shelf_pose = []
    for i in data['shelf_pose']:
        shelf_pose.append(i)
    # Closing file
    f.close()
    return data['grasped_obj_name'], init, goal, obstacle_name, obstacle_pose, shelf_name, shelf_pose
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', type=str, default='shelving')
    ...
    contact_model = ContactDiffusion(args.contact_model_diffusion_step, args.contact_model_scheduler).float()
    ...
    dirname = 'result/tohgs_graspdiffusion_contactdiffusion_shelving_{}'.format(time_str)
    ...
    for k, obj in enumerate(obj_list):
        ...
        print(config)
        grasped_obj, init, goal, obstacle_names, obstacle_poses, shelf_names, shelf_poses = get_json(config)
        ...
        obj_good_ratio = []
        output = {'init_hand_vertex':[], 'goal_hand_vertex':[], 'init_pose':[],
                    'goal_pose':[], 'obstacle_name': [], 'obstacle_pose': [], 'grasped_obj':grasped_obj[0],
                    'shelf_name': [], 'shelf_pose': [], 'cmap': [], 'obj_pc': []}
        print('obj: {}'.format(obj))
        for i in range(50, len(init), 10):
            init_grasped_pose = np.eye(4)
            init_grasped_pose[:3, :3] = Rot.from_quat(
                np.array(init[i][3:])).as_matrix()
            init_grasped_pose[:3, 3] = np.array(init[i][:3]).T

            goal_grasped_pose = np.eye(4)
            goal_grasped_pose[:3, :3] = Rot.from_quat(
                np.array(goal[i][3:])).as_matrix()
            goal_grasped_pose[:3, 3] = np.array(goal[i][:3]).T

            init_grasped_obj_mesh = trimesh.load(os.path.join('models', 'meshdata', grasped_obj[i], 'coacd', 'decomposed_scaled.obj'))
            init_grasped_obj_mesh.apply_transform(init_grasped_pose)
            init_grasped_obj_vox = init_grasped_obj_mesh.voxelized(pitch=0.001)
            goal_grasped_obj_mesh = trimesh.load(os.path.join('models', 'meshdata', grasped_obj[i], 'coacd', 'decomposed_scaled.obj'))
            ...
            obs_mesh = [table]
            for j in range(len(obstacle_names[i])):
                name = obstacle_names[i][j]
                temp = np.array(obstacle_poses[i][j])
                pose = np.eye(4)
                ...
                sample = trimesh.sample.sample_surface(mesh, args.point_sample_num)[0]
                obstacles_pcd_list.append(torch.FloatTensor(sample))
                pyt3d_mesh = pytorch3d.structures.Meshes(torch.FloatTensor(mesh.vertices).unsqueeze(0), torch.FloatTensor(mesh.faces).unsqueeze(0)).cuda()
                obstacle_mesh.append(pyt3d_mesh)
                obs_mesh.append(mesh)
            shelf_pose = np.eye(4)
            shelf_pose[:3, :3] = Rot.from_quat(
                np.array(shelf_poses[i][3:])).as_matrix()
            shelf_pose[:3, 3] = np.array(shelf_poses[i][:3]).T
            shelf_mesh = trimesh.load(os.path.join('models','selected_shelf', shelf_names[i], 'coacd', 'decomposed.obj'))
            shelf_mesh.apply_transform(shelf_pose)
            obs_mesh.append(shelf_mesh)
            pyt3d_mesh = pytorch3d.structures.Meshes(torch.FloatTensor(shelf_mesh.vertices).unsqueeze(0), torch.FloatTensor(shelf_mesh.faces).unsqueeze(0)).cuda()
            obstacle_mesh.append(pyt3d_mesh)
            ...
            with torch.no_grad():
                dummy_input = torch.ones((args.cmap_sample_num, 2048)).to(device).float()
                recon_h2o_cmap, _ = contact_model.p_sample_loop({'x':dummy_input, 'init_obj_verts': init_grasped_obj_mesh_pcd[:, :, :3], 'goal_obj_verts': goal_grasped_obj_mesh_pcd[:, :, :3], 'scene_pcd': scene_pcd, 'origin_obj_verts': origin_grasped_obj_mesh_pcd, 'origin_obj_normal': origin_obj_normal})
            ...
            for s in range(args.cmap_sample_num * args.grasp_sample_num):
                ...
                for index in range(len(qr_threshold)):
                    if penetr_vol < qr_threshold[index][0] and simu_disp < qr_threshold[index][1]:
                        ...
                        qr[index].append(0)
                output['init_pose'].append(init[i])
                output['goal_pose'].append(goal[i])
                output['obstacle_name'].append(obstacle_names[i])
                output['obstacle_pose'].append(obstacle_poses[i])
                output['shelf_name'].append(shelf_names[i])
                output['shelf_pose'].append(shelf_poses[i])
                ...
                # output['obj_pc'] = 
                # output['cmap'].append(recon_h2o_cmap[s].cpu().numpy())
                if args.vis:
                    ...
                    a = trimesh.Scene([init_pcd])
                    a.set_camera(angles=[np.pi/3, 0, 0], distance=1.0)
                    png = a.save_image(visible=True)
                    file_name = os.path.join(dirname, 'image', "{}_{}_{}_contact.png".format(obj, i, s))
                    ...
                    a.set_camera(angles=[np.pi/4, 0, 0], center=[0, -0.1, 0.8])
                    png = a.save_image(visible=True)
                    file_name = os.path.join(dirname, 'image', "{}_{}_init_scene_contact.png".format(obj, i, s))
                    with open(file_name, 'wb') as f:
                        f.write(png)
                        f.close()
                    ...
                    a.set_camera(angles=[np.pi/4, 0, 0], center=[0, -0.1, 0.8])
                    png = a.save_image(visible=True)
                    file_name = os.path.join(dirname, 'image', "{}_{}_goal_scene_contact.png".format(obj, i, s))
                    with open(file_name, 'wb') as f:
                        f.write(png)
                        f.close()

                    a = trimesh.Scene([init_pcd])
                    a.set_camera(angles=[np.pi/3, 0, 0], distance = 1)
                    png = a.save_image(visible=True)
                    file_name = os.path.join(dirname, 'image', "{}_{}_{}_contact.png".format(obj, i, s))
                    with open(file_name, 'wb') as f:
                        f.write(png)
