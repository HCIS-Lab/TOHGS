...
def get_json(filename):
    ...
    # Closing file
    f.close()
    return data['grasped_obj_name'], init, goal, obstacle_name, obstacle_pose
...
if __name__ == '__main__':
    ...
    dirname = 'result/tohgs_grasodiffusion_contactdiffusion_placing_{}'.format(time_str)
    ...
    for k, obj in enumerate(obj_list):
        ...
        grasped_obj, init, goal, obstacle_name, obstacle_pose = get_json(config)
        ...
        output = {'init_hand_vertex':[], 'goal_hand_vertex':[], 'init_pose':[],
                    'goal_pose':[], 'obstacle_name': [], 'obstacle_pose': [], 'grasped_obj':grasped_obj}
        ...
        for i in range(len(init)):
            ...
            init_grasped_obj_mesh = trimesh.load(os.path.join('models', 'meshdata', grasped_obj, 'coacd', 'decomposed_scaled.obj'))
            init_grasped_obj_mesh.apply_transform(init_grasped_pose)
            init_grasped_obj_vox = init_grasped_obj_mesh.voxelized(pitch=0.001)
            goal_grasped_obj_mesh = trimesh.load(os.path.join('models', 'meshdata', grasped_obj, 'coacd', 'decomposed_scaled.obj'))
            ...
            obs_mesh = [table]
            for j in range(len(obstacle_name[i])):
                name = obstacle_name[i][j]
                temp = np.array(obstacle_pose[i][j])
                pose = np.eye(4)
            ...
            with torch.no_grad():
                ...
                recon_h2o_cmap, _ = contact_model.p_sample_loop({'x':dummy_input, 'init_obj_verts': init_grasped_obj_mesh_pcd[:, :, :3], 'goal_obj_verts': goal_grasped_obj_mesh_pcd[:, :, :3], 'scene_pcd': scene_pcd, 'origin_obj_verts': origin_grasped_obj_mesh_pcd})
            ...
            # print(successful_flag.sum() / args.grasp_sample_num)
            ...
            for s in range(args.cmap_sample_num * args.grasp_sample_num):
                ...
                try:
                    simu_disp = run_simulation(init_hand_vertex[s].detach().cpu().squeeze(0).numpy()  - trans.cpu().numpy() , rh_faces.cpu().numpy().reshape((-1, 3)),
                                            init_grasped_obj_mesh.vertices - trans.cpu().numpy(), init_grasped_obj_mesh.faces,
                                            vhacd_exe=vhacd_exe, save_video=False, save_video_path=os.path.join(dirname,"{}_{}_{}_sd.gif".format(obj, i, s)),
                                            use_gui=False)
                except:
                    simu_disp = 0.10
                    print('vhacd error')
                ...
                output['obstacle_name'].append(obstacle_name[i])
                output['obstacle_pose'].append(obstacle_pose[i])
                ...
                if args.vis:
                    ...
                    a.set_camera(angles=[np.pi/4, 0, 0], center=[0, -0.1, 0.8])
                    png = a.save_image(visible=True)
                    file_name = os.path.join(dirname, 'image', "{}_{}_{}init_scene_contact.png".format(obj, i, s))
                    with open(file_name, 'wb') as f:
                        f.write(png)
                        f.close()
